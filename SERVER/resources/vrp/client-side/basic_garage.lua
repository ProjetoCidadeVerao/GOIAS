...
function tvRP.getCar(vehicle)
	for k,v in pairs(Garages.VehList) do
		if vehicle == v.name then 
			return v.nome,v.price,v.portamalas,v.hash,v.name,v.banido,v.vendivel
		end
	end
end 
...
function tvRP.ModelName(radius)
	...
	if IsEntityAVehicle(veh) then
		...
		local v = Garages.VehList[GetEntityModel(veh)]
		if v then
			return GetVehicleNumberPlateText(veh),v.model,VehToNet(veh),v.trunk,parseInt(v.price),lock,GetDisplayNameFromVehicleModel(v.name),GetStreetNameFromHashKey(GetStreetNameAtCoord(x,y,z)),veh
		end
	end
end
