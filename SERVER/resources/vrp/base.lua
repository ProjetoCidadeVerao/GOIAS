...
function vRP.isWhitelisted(user_id)
	...
	return rows[1].whitelist or false
end
...
local geral = {
	logo = "https://cdn.discordapp.com/attachments/1026982384438091856/1027360541439705098/LOGO-TEXAS-ROLEPLAY_sem_fundio.png", -- LOGO do Servidor
    ...
    discord = "https://discord.gg/TqcXGwxRe6", -- Discord do Servidor (Colocar https://)

    color = 6356736, -- Cor da Lateral do WeebHook
    footer = "© Goias RP", -- RODAPE do WeebHook
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local DeferralCards = exports["vrp"]:DeferralCards()
AddEventHandler("playerConnecting", function(name, setKickReason, deferrals,idfs)
	if not GeneratingCache then
		local cardCacheFail = DeferralCards.Card:Create({
			body = {
				DeferralCards.CardElement:Image({
					url = "https://cdn.discordapp.com/image",
					size = "large",
				...
				DeferralCards.CardElement:TextBlock({
					size = "ExtraLarge",
					weight = "Bolder",
					text = "Ops, houve um problema. tente novamente.",
					horizontalAlignment = "center",
					wrap = true
				}),
		...
		deferrals.presentCard(cardCacheFail, function(data, rawData)
			if data.submitId == "confirm" then
				deferrals.update("⛔ Saindo...")
				Wait(2500)
				deferrals.done("[Goiás Roleplay] Ops, houve um problema. tente novamente.")
				return
			end
		end)
		return
	...
	if ids == nil or #ids == 0 then
		local cardIdentity = DeferralCards.Card:Create({
			body = {
				DeferralCards.CardElement:Image({
					url = "https://cdn.discordapp.com/image",
					size = "large",
				...
				DeferralCards.CardElement:TextBlock({
					size = "ExtraLarge",
					weight = "Bolder",
					text = "Ocorreu um problema de identidade.",
					horizontalAlignment = "center",
					wrap = true
				}),
				DeferralCards.Container:ActionSet({
		...
		deferrals.presentCard(cardIdentity, function(data, rawData)
			if data.submitId == "confirm" then
				deferrals.update("⛔ Saindo...")
				Wait(2500)
				deferrals.done("[Goiás Roleplay] Ocorreu um problema de identidade.")
				return
			end
		end)
		return
	...
	if not user_id then
		local cardIdentification = DeferralCards.Card:Create({
			body = {
				DeferralCards.CardElement:Image({
					url = "https://cdn.discordapp.com/image",
					size = "large",
				...
				DeferralCards.CardElement:TextBlock({
					size = "ExtraLarge",
					weight = "Bolder",
					text = "Ocorreu um problema de identificação.",
					horizontalAlignment = "center",
					wrap = true
				}),
				DeferralCards.Container:ActionSet({
		...
		deferrals.presentCard(cardIdentification, function(data, rawData)
			if data.submitId == "confirm" then
				deferrals.update("⛔ Saindo...")
				Wait(2500)
				deferrals.done("[Goiás Roleplay] Ocorreu um problema de identificação.")
				return
			end
		end)
		return
	...
	if not vRP.isWhitelisted(user_id) then
		local cardWhitelist = DeferralCards.Card:Create({
			body = {
				DeferralCards.CardElement:Image({
					url = "https://cdn.discordapp.com/image",
					size = "auto",
				...
				DeferralCards.CardElement:TextBlock({
					size = "ExtraLarge",
					weight = "Bolder",
					text = "Bem vindo a Goiás RolePlay \n Para entrar no nosso servidor basta você acessar nosso discord e fazer a Whitelist \n Seu ID: [ "..user_id.." ]",
					horizontalAlignment = "center",
					wrap = true
				}),
				DeferralCards.Container:ActionSet({
					actions = {
						DeferralCards.Action:OpenUrl({
							url = "https://discord.gg/yaTVuWeNNP",
							title = "ACESSAR O DISCORD!"
						})
					}
				})
			}
		})
		deferrals.presentCard(cardWhitelist, function(data, rawData)
			return
		end)
		return
	end
	...
	local idfs = getIdentifiersLog(source)
	vRP.sendLog("entrada","[ENTROU NO SERVIDOR]\n[ID]: "..user_id.."\n[SOURCE]: "..source.." "..idfs)
...
function getIdentifiersLog(source)
	local source = source
	local ip = vRP.getPlayerEndpoint(source)
	local message = ""
	local identifiers = {
        steam = "",
        steamurl = "",
    ...
    for i = 0, GetNumPlayerIdentifiers(source) - 1 do
        ...
        elseif string.find(id, "ip") then
            identifiers.ipadress = string.gsub(id,"ip:","")
	...
	if identifiers.discordid ~= "" then message = message.."\n[DISCORD ID]: "..identifiers.discordid:gsub("discord:", "").."" else message = message.."\n[DISCORD ID]: N/A" end
	if identifiers.license ~= "" then message = message.."\n[LICENSE]: license:"..identifiers.license:gsub("license2:", "").."" else message = message.."\n[LICENSE]: N/A" end
	if identifiers.license2 ~= "" then message = message.."\n[LICENSE2]: "..identifiers.license2.."" else message = message.."\n[LICENSE2]: N/A" end
	if identifiers.fivem ~= "" then message = message.."\n[FIVEM]: "..identifiers.fivem.."" else message = message.."\n[FIVEM]: N/A" end
	if identifiers.steam ~= "" then message = message.."\n[STEAM ID]: "..identifiers.steam.."" else message = message.."\n[STEAM ID]: N/A" end
	if identifiers.steamurl ~= "" then message = message.."\n[STEAM URL]: https://steamcommunity.com/profiles/" ..tonumber(identifiers.steamurl:gsub("steam:", ""),16).."" else message = message.."\n[STEAM URL]: N/A" end
	if identifiers.xbl ~= "" then message = message.."\n[XBL]: "..identifiers.xbl.."" else message = message.."\n[XBL]: N/A" end
	if identifiers.live ~= "" then message = message.."\n[LIVE]: "..identifiers.live.."" else message = message.."\n[LIVE]: N/A" end
	if identifiers.ipadress ~= "" then message = message.."\n[IP]: "..identifiers.ipadress.."" else message = message.."\n[IP]: N/A" end
	return message
end
...
function vRP.log(file,info)
    file = io.open(file ..os.date("%d.%m.%Y")..".txt", "a")

    if file then
      file:write(os.date("%X").." » "..info.."\r")
    end
    file:close()
end
...
function vRP.dropPlayer(source,reason)
	...
	if user_id then
		...
		if loc == nil then loc = { x = 0, y = 0, z = 0 } end
		...
		vRP.sendLog("saida","[SAIU DO SERVIDOR]\n[ID]: "..user_id.."\n[SOURCE]: "..source.."\n[REASON]: "..reason.."\n[LOCALIZAÇÃO]: "..loc.x..","..loc.y..","..loc.z)
	end
end
