...
SystemConfig = {
	["License"] = "K2UYKOUTZGXO70PIG95IYTRDV8VHES", 				------ Licença.
	["UsePerm"] = false,			------ Define se irá usar permissão.
	...
	["PayAmount"] = {1000,5000},		------ Define quanto o player irá receber após reparo bem-sucedido. (Número inteiro: Recompensa fixa | Array: Recomensa randomica entre o 1 e 2 valor)
	["Car"] = "unofirma",			------ Modelo do carro usado para pegar escadas.
	...
	["BlackoutItem"] = "maconha",	------ Define o item a ser utilizado.
...
Translate = {
	...
	["BlackoutWarn"] = "O fornecimento de energia da cidade foi danificado!",
	...
	["MoveKeys"] = "~r~4,5,6 E 8~w~ ~g~PARA MOVIMENTAR O OBJETO",
	["RotKeys"] = "~r~7 E 9~w~ ~y~PARA ROTACIONAR O OBJETO",
	["SaveKey"] = "~r~+~w~ ~b~PARA SALVAR CORDENADA",
...
GetPerm = function(user_id)
	return vRP.hasPermission(user_id,SystemConfig.Perm)
end

--- Remover dinheiro (multa)

TryMoney = function(user_id,amount)
...
GetItem = function(user_id)
	return vRP.tryGetInventoryItem(user_id,SystemConfig["BlackoutItem"],1)
end
-----------------------------------------------------------------------------
-------------------------Client Functions------------------------------------
-----------------------------------------------------------------------------
