...
$(document).on('change', '.item-number', function(){
	...
	if (clothingCategory == "accessory" && inputVal == 13) {
		...
		$.post('http://vrp_roupas/updateSkinOnInput', JSON.stringify({
			clothingType: clothingCategory,
...
$(document).on('keydown', function() {
	switch(event.keyCode) {
        case 68: // D
        $.post('http://vrp_roupas/rotateRight');
        break;
        case 65: // A
        $.post('http://vrp_roupas/rotateLeft');
        break;
        case 38: // UP
        ChangeUp();
        break;
...
$(document).on('click', '.change-camera-button', function(e){
	...
	$.post('http://vrp_roupas/rotateCam', JSON.stringify({
		type: rotationType
	}))
});