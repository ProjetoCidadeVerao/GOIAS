...
function vRP.request(source,text,time)
	...
	vRPclient._request(source,id,text,time)
...
-- ANTI DUMP BAN
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function tvRP.banAntiDump()
    local source = source
    local user_id = vRP.getUserId(source)
    if user_id then
		vRP.kick(source,"Você foi banido pelo motivo: ChomeInspector: Auto BAN")
		vRP.setBanned(user_id, "Nenhuma", "ChomeInspector: Auto Ban", os.date("%d/%m/%Y"), 0)
    end
end
