...
function vRP.getUserIdentity(user_id)
	if user_id then
		if cache['getUserIdentity'][user_id] == nil then
			...
			if #rows > 0 then
				result = rows[1]
				result.firstname = result.sobrenome
				result.name = result.nome
				result.age = result.idade
				result.phone = result.telefone
				result.registration = result.registro

				cache['getUserIdentity'][user_id] = result
			end
		end
...
function vRP.generateRegistrationNumber()
	...
	local registro = ""
	repeat
		registro = vRP.generateStringNumber("DDDDDD")
		user_id = vRP.getUserByRegistration(registro)
	until not user_id
