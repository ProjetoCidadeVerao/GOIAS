...
Citizen.CreateThread(function()
	while true do
        local time = 600
        local ped = PlayerPedId()
        if IsEntityPlayingAnim(ped,"anim@amb@nightclub@peds@","rcmme_amanda1_stand_loop_cop",3) or IsEntityPlayingAnim(ped,"anim@mp_player_intupperface_palm","idle_a",3) or IsEntityPlayingAnim(ped,"anim@heists@heist_corona@single_team","single_team_loop_boss",3) or IsEntityPlayingAnim(ped,"anim@mp_player_intselfiethe_bird","idle_a",3) or IsEntityPlayingAnim(ped,"random@arrests@busted","idle_a",3) or IsEntityPlayingAnim(ped,"random@mugging3","handsup_standing_base",3) or IsEntityPlayingAnim(ped,"mini@strip_club@idles@bouncer@base","base",3) or IsEntityPlayingAnim(ped,"rcmnigel1c","hailing_whistle_waive_a",3) or IsEntityPlayingAnim(ped,"anim@mp_player_intselfiethumbs_up","idle_a",3) or IsEntityPlayingAnim(ped,"anim@mp_player_intcelebrationmale@face_palm","face_palm",3) or IsEntityPlayingAnim(ped,"anim@mp_player_intcelebrationmale@salute","salute",3) or IsEntityPlayingAnim(ped,"amb@prop_human_parking_meter@female@idle_a","idle_a_female",3) then
            time = 500
...
AddEventHandler("Notify",function(css,mensagem,timer)
	if timer == nil or timer >= 1000 then
		timer = 5
	end

	SendNUIMessage({ css = css, mensagem = mensagem, timer = timer * 1000 })
end)
...
AddEventHandler("NotifyItem",function(mode, quantidade, itemname, idname, kgTotal)
...
RegisterNetEvent("OpenNotifyPush")
AddEventHandler("OpenNotifyPush",function(data)
...
RegisterNUICallback("setWay",function(data)
	...
	TriggerEvent("Notify","sucesso","Você marcou a rota no mapa.", 5)
end)
...
RegisterCommand('emote', function(source,args)
	if not IsPauseMenuActive() then
		local ped = PlayerPedId()
		if GetEntityHealth(ped) > 105 and vRPserver.checkCommandsC() and not celular_state and not menu_state.opened then
			if not IsPedInAnyVehicle(ped) then
				...
				elseif args[1] == "joia2" then
					if IsEntityPlayingAnim(ped,"anim@mp_player_intselfiethumbs_up","idle_a",3) then
						tvRP.DeletarObjeto()
					else
...
RegisterKeyMapping("emote dedo","Executar Animação","keyboard","f3")
RegisterKeyMapping("emote assobiar","Executar Animação","keyboard","down")
RegisterKeyMapping("emote joia2","Executar Animação","keyboard","left")
