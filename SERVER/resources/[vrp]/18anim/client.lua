...
local function DetachPlayer(playerPed)
    if IsEntityAttachedToAnyPed(playerPed) then DetachEntity(playerPed) end
end
...
local function IsAlive(playerPed)
    return (GetEntityHealth(playerPed) > 101)
end
...
local function GetClosestPlayer()
    ...
    for i = 1, #activePlayers do
        if activePlayers[i] == playerId then goto skip_loop end
        local targetPed = GetPlayerPed(activePlayers[i])
        if not PedCheck(targetPed) then goto skip_loop end
...
local function LoadAnimDict(dict)
    if type(dict) == "table" then
        for i = 1, #dict do LoadAnimDict(dict[i]) end
    else
...
local function UnloadAnimDict(dict)
    if type(dict) == "table" then
        for i = 1, #dict do RemoveAnimDict(dict[i]) end
    else RemoveAnimDict(dict) end
end
...
function ANIMS:GetAnimTask(playerPed, isSource)
    if CheckAnimFlag(self.action, "isVehicleAnim") then
        ...
        if vehicle ~= 0 and DoesEntityExist(vehicle) then
            ...
            local found = false
            for i = 1, #seats do
                if (GetPedInVehicleSeat(vehicle, seats[i]) == playerPed) then
                    found = true
                    break
                end
            end
    ...
    if isSource then return _ANIM[self.action].source
    else return _ANIM[self.action].target end
end
...
function ANIMS:Stop(syncOther, skipStop)
    if self.active then
        if syncOther then
            ...
            if otherPlayer then TriggerServerEvent("randallfetish:syncStopAnim", GetPlayerServerId(otherPlayer), self.action) end
        end
        local playerPed = PlayerPedId()
        ...
        if self.freeze then FreezeEntityPosition(playerPed, false) end
    end
    self.active  = false
    self.freeze  = false
    self.action  = nil
...
function ANIMS:Start(otherPlayer, isSource, action)
    ...
    if not IsAlive(playerPed) or not PedCheck(targetPed) then ShowNotification(_s("anim_error")) return end
    if IsPedInAnyVehicle(playerPed, true) ~= IsPedInAnyVehicle(targetPed, true) then ShowNotification(_s("both_none_vehicle")) return end
    if isSource then self.target = otherPlayer
    else self.source = otherPlayer end
    self.action  = action
...
function ANIMS:ChangeSpeed(increase)
    ...
    if increase then newSpeed = newSpeed + 0.1
    else newSpeed = newSpeed - 0.1 end
    if newSpeed > 1.7 then ShowNotification(_s("max_speed_alert"))
    elseif newSpeed < 0.6 then ShowNotification(_s("min_speed_alert"))
    else
...
function REQUEST:new(action)
	if self.active then ShowNotification(_s("pending_request")) return end
	local closestPlayer, closestDistance = GetClosestPlayer()
	if closestPlayer == -1 or closestDistance > 1.5 then ShowNotification(_s("player_not_found")) return end
	self.active = true
...
function REQUEST:receive(playerId, action)
	if self.active then ShowNotification(_s("pending_request_2")) return end
	...
	local timeout = 15
	CreateThread(function()
		local requestHint = _s("request_prompt", Configx.Controls.Accept[1], Configx.Controls.Refuse[1], animData.label)
		local acceptKey = Configx.Controls.Accept[2]
		local refuseKey = Configx.Controls.Refuse[2]
		while self.active do
			Wait(0)
	...
	CreateThread(function()
		while self.active do
			...
			local cancel = false
			timeout = timeout - 1
			if timeout <= 0 or not PedCheck(playerPed) or not PedCheck(sourcePed) then cancel = true
			else
				local playerPos = GetEntityCoords(playerPed)
				local sourcePos = GetEntityCoords(sourcePed)
				if #(playerPos - sourcePos) > 5.0 then cancel = true end
			end
			if cancel then
				if self.active then
					self.active = false
...
function REQUEST:accepted(playerId)
	...
	if otherPlayerId ~= playerId then print("^1Player ID missmatch, cancelling animation request^7") return end
	local isSource = (self.target ~= nil)
	local otherPlayer = GetPlayerFromServerId(otherPlayerId)
	if otherPlayer == -1 then ShowNotification(_s("anim_error")) return end
...
AddEventHandler("randallfetish:receiveRequest", function(playerId, action)
	local playerPed = PlayerPedId()
	if IsAlive(playerPed) then REQUEST:receive(playerId, action) end
end)
...
RegisterCommand(Configx.Command, function(source, args)
    local index = tonumber(args[1])
    if not index then ShowNotification(_s("invalid_command", Configx.Command)) return
    elseif not _ANIM[index] then ShowNotification(_s("invalid_command_2", index)) return
    else
        local playerPed = PlayerPedId()
        if not IsAlive(playerPed) then ShowNotification(_s("cant_perform_dead")) return end
        if IsPedInAnyVehicle(playerPed, true) and not CheckAnimFlag(index, "isVehicleAnim") then ShowNotification(_s("off_vehicle_only")) return end
    end
    REQUEST:new(index)
end)
...
AddEventHandler("onResourceStop", function(resource)
    ...
    if not (ANIMS.active) then return end
    local playerPed = PlayerPedId()
