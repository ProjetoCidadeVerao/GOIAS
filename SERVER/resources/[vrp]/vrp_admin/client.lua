...
-----------------------------------------------------------------------------------------------------------------------------------------
-- VEICULOS EXPOSIÇÃO FLACLAN#7101
-----------------------------------------------------------------------------------------------------------------------------------------
local CDSPrincipalX = -192.13 
local CDSPrincipalY = -1329.66
local CDSPrincipalZ = 30.74

local coordsexp = {
    [1] = {   ['modelo'] = 'panamera18', ['x'] = -192.13 , ['y'] = -1329.66, ['z'] =  30.74, ['h'] = 359.07 },  
    [2] = {   ['modelo'] = 'mclarensenna', ['x'] = -203.16, ['y'] = -1332.67, ['z'] = 34.5, ['h'] = 232.12 },  
    [3] = {   ['modelo'] = 'zx10r', ['x'] =-191.51, ['y'] = -1333.72, ['z'] = 34.44, ['h'] = 93.28 }, 
...
Citizen.CreateThread(function()
    while true do
        for k,v in pairs(coordsexp) do
            local ped = PlayerPedId()
            local coords = GetEntityCoords(ped)
            local distance = #(coords - vector3(CDSPrincipalX,CDSPrincipalY,CDSPrincipalZ))
            if distance <= 100 then
                if vehicleexp[v.modelo] == nil then
                    RequestModel(GetHashKey(v.modelo))
                    while not HasModelLoaded(GetHashKey(v.modelo)) do
                        Wait(1)
                    end
                    local car = CreateVehicle(GetHashKey(v.modelo), v.x, v.y, v.z, v.h, false, false)
                    FreezeEntityPosition(car, true)
                    SetEntityInvincible(car, true)
                    SetEntityCoords(car, v.x, v.y, v.z, false, false, false, true)
                    SetVehicleDoorsLocked(car,2)
                    vehicleexp[v.modelo] = true
                end
            end
        end
        Citizen.Wait(5000)
...
-- EFEITO LGBT
-----------------------------------------------------------------------------------------------------------------------------------------
function src.effectLGBT()
	local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1)))
    local particleDictionary = "scr_rcbarry2"
    local particleName = "scr_clown_bul"
    RequestNamedPtfxAsset(particleDictionary)
    while not HasNamedPtfxAssetLoaded(particleDictionary) do
    Citizen.Wait(0)
    end
    SetPtfxAssetNextCall(particleDictionary)
    local effect = StartParticleFxLoopedAtCoord(particleName, x, y, z+0.5, 0.0, 180.0, 0.0, 2.0, false, false, false, false)
end
...
-- SPAWNAR VEICULO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('spawnarveiculo12')
AddEventHandler('spawnarveiculo12',function(name)
...
AddEventHandler('ObjectDeleteGunOn', function()
	...
	toggle = true
	TriggerClientEvent("Notify",source,"sucesso","Você ativou a arma de remoção de objetos.",5)
end)

AddEventHandler('ObjectDeleteGunOff', function()
	toggle = false
	TriggerClientEvent("Notify",source,"negado","Você desativou a arma de remoção de objetos.",5)
end)
...
AddEventHandler("vehtuning",function(vehicle)
	...
	if IsEntityAVehicle(vehicle) then
		...
		--SetVehicleNumberPlateText(vehicle,"LUX")
	...
	while true do
		...
		if prisioneiro then
			...
			if distance >= 25 then
				...
				--TriggerClientEvent("Notify",source,"negado","Você não pode sair da PRISÃO ADMINISTRATIVA.",5)
			end
		end
	end
end)
...
-- PLACA MERCOSUL
----------------------------------------------------------------------------------------------------------------------------------------
imageUrl = "https://cdn.discordapp.com/attachments/750574567868596224/883450210297208862/1212.png" -- 
