...
RegisterNetEvent("wall:Blips:LoadAdminMode",function(userConfig)
    if inAdminMode then
        ...
        TriggerEvent("Notify","negado","Wall desativados.")
    else
        config = userConfig
        inAdminMode = true
        serverAPI.loadInfos()
        startModule()
        TriggerEvent("Notify","sucesso","Wall ativados.")
    end
end)
