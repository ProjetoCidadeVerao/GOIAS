...
AddEventHandler("vRP:playerSpawn",function(user_id,source)
    ...
    if identity then
        playerList[source] = { 
            ...
            isStaff = vRP.hasPermission(user_id, "perm.wall"),
            orgName = vRP.getUserGroupOrg(user_id) or "wall"
        }
    else
        playerList[source] = { 
            steamName = GetPlayerName(source), 
...
local function forceSync()
    for k,v in pairs(vRP.getUsers()) do
        ...
        local isStaff = vRP.hasPermission(k,"perm.wall")
        ...
        playerList[v] = {
            ...
            name = identity.nome.." "..identity.sobrenome, 
            staff = isStaff,
            orgName = vRP.getUserGroupOrg(k) or "wall"
        }
    end
...
RegisterCommand("wall",function(source,args,rawCmd)
    local user_id = vRP.getUserId(source)
    local identity = vRP.getUserIdentity(user_id)
    if not vRP.hasPermission(user_id,"perm.wall") then
        return
    end
...
RegisterCommand('wall2',function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
        if not vRP.hasPermission(user_id, "perm.wall") or not adminList[user_id] then return false end
