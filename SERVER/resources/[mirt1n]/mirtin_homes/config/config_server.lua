...
config.nationgarages = false -- Caso use nation garages coloque true.

config.lotus = true
config.iptuValue = 0.05 -- 5% preço da propriedade
config.iptuVencimento = 30 -- Dias Antes do SELLHOUSE para liberar o pagamento do IPTU ** ATENCAO **
config.sellHouseIptu = 60 -- Dias que a Casa vai ser vendida 
...
config.ipvaVencimento = 30 -- Dias para o vencimento do IPVA ( CASO USE MEU SISTEMA DE GARAGEM )
    ...
    if user_id then
        if vRP.hasPermission(user_id, "developer.permissao") then -- permissao para deletar a casa
            ...
            if idProp ~= nil and tonumber(idProp) then
                if RegisterTunnel.checkIdHouse(idProp) then
...
RegisterCommand('criargaragem', function(source,args) -- Comando deletarcasa
    ...
    if user_id then
        ...
        if vRP.hasPermission(user_id, "developer.permissao") or vRP.hasGroup(user_id, "VALEGARAGEM") then -- permissao para deletar a casa
            local idProp = vRP.prompt(source, "Digite o ID da propriedade: ", "")
            if idProp ~= nil and tonumber(idProp) then
                if RegisterTunnel.checkIdHouse(idProp) then
                            ...
                            for k,v in pairs(owner) do
                                if parseInt(idProp) == parseInt(v.houseID) then
                                    ...
                                    if vRP.request(source, "Posicione o veiculo e fique dentro dele", 60) then
                                        local spawnCoords = vTunnel.getSpawnVehicle(source)
                                        if spawnCoords then
...
RegisterCommand('home', function(source,args) -- Comando gerais das casas
    ...
    if user_id then
        ...
        if args[1] == "list" then
            ...
            local props = ""
            for k,v in pairs(owner) do
                props = props.."^2(ID: "..v.houseID..")^0 localizada em ^2 "..vTunnel.getStreetName(source, propriedades[v.houseID].coords.x,propriedades[v.houseID].coords.y,propriedades[v.houseID].coords.z).."^0 .\n"
            end
        ...
        if args[1] == "iptu" and tonumber(args[2]) then
            local owner = RegisterTunnel.checkIsOwner(user_id, tonumber(args[2]))
            if not owner then
        ...
        if args[1] == "disp" then
            ...
            vTunnel.allDispHouses(source)
            return
        end

        if args[1] == "moradores" and tonumber(args[2]) then
            local moradores = RegisterTunnel.getMoradores(user_id, tonumber(args[2]))
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RegisterTunnel.comprarPropriedade = function(id, tipo)
    local source = source
    local user_id = vRP.getUserId(source)
    if user_id then
...
RegisterTunnel.interfone = function(id) 
    local source = source
    local user_id = vRP.getUserId(source)
    ...
    if user_id then
        ...
        if interfone ~= nil and tonumber(interfone) then
            local ap = RegisterTunnel.checkEnterAp(parseInt(user_id), id, parseInt(interfone))
            if ap == 1 then
                vTunnel.enterAP(source, id, interfone)
                return
            end
...
-- SISTEMA DA GARAGEM
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RegisterTunnel.myVehicles = function(user_id) -- Captura seus Veiculos
...
RegisterTunnel.isMyVehicle = function(user_id, veiculo) -- Verifica se o veiculo realmente existe na garagem dele
    local source = vRP.getUserSource(user_id)
...
RegisterTunnel.getCustomVehicle = function(user_id, veiculo) -- Captura a tunagem do veiculo
    local source = vRP.getUserSource(user_id)
    local myVehs = vRP.query("mirtin_homes/isMyVehicle", {user_id = user_id, veiculo = veiculo})
    if #myVehs == 0 then
...
-- SISTEMA DE BAU
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RegisterTunnel.openBau = function(user_id, id, houseId)
    local source = vRP.getUserSource(user_id)
...
-- SISTEMA DO ARMARIO
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RegisterTunnel.CsalvarRoupa = function(id)
    local source = source 
    local user_id = vRP.getUserId(source)
    if user_id then
        ...
        if RegisterTunnel.salvarRoupas(id, nameRoupa, vRPclient.getCustomization(source)) then
            TriggerClientEvent("Notify",source,"sucesso","Você guardou a roupa <b>"..nameRoupa.."</b> no armario.", 5000)
        end
    end
end

RegisterTunnel.CuseRoupas = function(user_id, name, custom)
    local source = vRP.getUserSource(user_id)
...
RegisterTunnel.CdeletarRoupa = function(user_id, name)
    local source = vRP.getUserSource(user_id)
    TriggerClientEvent("Notify",source,"negado","Você apagou a roupa <b>"..name.."</b> do armario.", 5000)
end

RegisterTunnel.ServerConfig = function()
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RegisterTunnel.CaddMorador = function(user_id, id, houseID)
    local source = vRP.getUserSource(user_id)
    ...
    RegisterTunnel.addMorador(id, tonumber(idMorador), identity)
...
RegisterTunnel.CremoveMorador = function(user_id, id,houseID,morador)
    local source = vRP.getUserSource(user_id)
