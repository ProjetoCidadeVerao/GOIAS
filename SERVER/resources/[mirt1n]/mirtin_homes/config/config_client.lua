...
CreateThread(function()
    sv_config = vTunnel.ServerConfig()
end)
...
config.imagemDir = "http://177.54.148.31:4020/base/carros/" -- Diretorio das imagens dos veiculos
...
-- CONFIGS GARAGEM
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RegisterTunnel.tuningVehicle = function(custom, veh) -- Sua função de aplicar tunagem
	TriggerServerEvent("nation:syncApplyMods", custom,VehToNet(veh))
end

RegisterTunnel.deleteVehicle = function(veh) -- Sua função de deletar veiculo
...
-- CONFIGS BLIPS
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RegisterTunnel.allDispHouses = function() -- Blips do comando /home disp
...
RegisterTunnel.myHouseBlip = function(coords) -- Blips da propriedades dos players
            ...
            if distance <= 2.0 then
                ...
                if infoInterior.tipo == "casa" then
                    if IsControlJustReleased(1, 182) and segundos <= 0 then
                        ...
                        if vTunnel.checkEnterHouse(infoInterior.houseID) then
                            ...
                            vTunnel.syncLock(infoInterior.houseID, propriedades[infoInterior.houseID].porta)
                        end
                    end
                    ...
                    if IsControlJustReleased(1, 51) and segundos <= 0 then
                        ...
                        if not propriedades[infoInterior.houseID].porta then
                            vTunnel.sairPropriedade(infoInterior.houseID)
                        else
                        ...
                        if distance <= 2.0 then
                            if nearestHouse[k].tipo == "casa" then
                                if IsControlJustReleased(1, 182) and segundos <= 0 then
                                    ...
                                    if vTunnel.checkEnterHouse(k) then
                                        ...
                                        vTunnel.syncLock(k, nearestHouse[k].porta)
                                    end
                                end
                                ...
                                if IsControlJustReleased(1, 51) and segundos <= 0 then
                                    ...
                                    if not nearestHouse[k].porta then
                                        vTunnel.entrarPropriedade(k, nearestHouse[k].tipo)
                                    else
                                        if vTunnel.comprarPropriedade(k, tostring(nearestHouse[k].tipo)) then
                                            config.lang['trancada']()
                                        end
                                    end
                                end
                            ...
                            elseif nearestHouse[k].tipo == "apartamento" then
                                ...
                                if IsControlJustReleased(1, 145) and segundos <= 0 then
                                    segundos = 3
                                    vTunnel.comprarPropriedade(k, tostring(nearestHouse[k].tipo))
                                end
                            end
                        end
                    end
