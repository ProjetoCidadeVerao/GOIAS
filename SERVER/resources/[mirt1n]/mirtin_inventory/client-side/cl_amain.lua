...
RegisterCommand("invsee",function(source,args)
	if not IsPlayerFreeAiming(PlayerPedId()) and GetEntityHealth(PlayerPedId()) > 105 and myRevistar == nil and NetworkIsSessionActive() and segundos <= 0 and not vRP.isHandcuffed() and not IsPedInAnyVehicle(PlayerPedId()) then
		...
		if revistar and vSERVER.checkPermissioninvsee() then
			vSERVER._setRevistar(revistar, true)
			myRevistar = { revistar }
			SetNuiFocus(true, true)
			TransitionToBlurred(1000)
...
Citizen.CreateThread(function()
    while true do
		...
		for k,v in pairs(cfg.chestOrgs) do --//TODO
			local distance = #(coords - vector3(v.coords.x, v.coords.y, v.coords.z))
			if distance <= 2.5 then
				time = 5
				DrawText3D(v.coords.x,v.coords.y,v.coords.z,"Pressione ~g~E~w~ para abrir")
			end
		end
...
RegisterNetEvent('inventory:useDrugs', function(drug)
	...
	if drug == "maconha" or drug == "heroina" then
		...
		TriggerEvent("Notify","sucesso","Esta droga curou 25% de sua vida.", 5)
	end

	if drug == "cocaina" or drug == "lsd" then
		local armour = GetPedArmour(ped)
		SetPedArmour(ped, armour + 25)

		if GetPedArmour(ped) >= 100 then
			SetPedArmour(ped, 100)
		end

		TriggerEvent("Notify","sucesso","Esta droga curou 25% de seu colete.", 5)
	end

	if drug == "lancaperfume" or drug == "balinha" then
		countdownDrug = 15
		SetRunSprintMultiplierForPlayer(PlayerId(),1.15)
		TriggerEvent("Notify","sucesso","Esta droga aumentou sua velocidade.", 5)
		CreateThread(function()
			while countdownDrug >= 0 do
